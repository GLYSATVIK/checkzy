(() => {
  ...
  chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
    if (msg.action === "OPEN_DIALOG") {
      ...
      if (text) {
        ...
        if (!rect)
          rect = { bottom: window.innerHeight / 2, left: window.innerWidth / 2, top: 0, right: 0, width: 0, height: 0, x: 0, y: 0, toJSON: () => {
          } };
        showDialog(rect);
      }
    }
  ...
  function handleSelection() {
    ...
    setTimeout(() => {
      ...
      let text = "";
      let rect = null;
      if (shadowRoot && shadowRoot.contains(active))
        return;
  ...
  function showToolbar(rectOrRange) {
    if (!shadowRoot)
      initUI();
    if (!host)
      return;
    ...
    let rect;
    if (rectOrRange.getBoundingClientRect)
  ...
  function showDialog(rectOverride) {
    if (!shadowRoot)
      initUI();
    if (!host)
      return;
  ...
  function renderPresetsState() {
    ...
    chrome.storage.local.get(["openai_key", "gemini_key", "anthropic_key"], (res) => {
      const select = shadowRoot?.querySelector("#wp-model");
      if (!select)
        return;
      if (res.openai_key) {
        select.value = "gpt-4o";
      } else if (res.gemini_key) {
        select.value = "gemini-1.5-flash";
      } else if (res.anthropic_key) {
        select.value = "claude-3-haiku";
      } else {
        select.value = "gpt-4o";
      }
    });
  ...
  function renderResultState(text) {
    if (!dialog)
      return;
    currentResult = text;
    const body = dialog.querySelector(".wp-body");
  ...
  function bindDialogEvents() {
    ...
    const input = dialog.querySelector("#wp-custom-prompt");
    const sendBtn = dialog.querySelector(".wp-send-btn");
    const handleSend = () => {
      if (input.value.trim())
        generateText(input.value.trim());
    };
  ...
  async function generateText(prompt) {
    const text = window._checkzy_text || "";
    if (!text)
      return;
    if (!isExtensionValid()) {
      alert("Extension invalid. Please refresh page.");
      return;
    }
    renderLoadingState();
    const modelSelect = shadowRoot?.querySelector("#wp-model");
    ...
    try {
      ...
      if (res.error)
        ...
        renderResultState(res.text);
    } catch (e) {
      if (e.message && e.message.includes("invalidated"))
        renderError("Context Invalidated. Refresh page.");
      else
